...
public class MediaUploadService {
    ...
    public ResponseResult register(String fileMd5, String fileName, Long fileSize, String mimetype, String fileExt) {
        ...
        //做一些准备工作，检查文件所在的目录是否存在，如果不存在则创建
    ...
    //合并文件
    private File mergeFile(List<File> file,File mergeFile){
        try {
            //如果合并文件已存在，则删除，否则创建新文件
            if(mergeFile.exists()){
                mergeFile.delete();
            }else{
                //创建新文件
    ...
    public ResponseResult sendProcessVideoMsg(String mediaId){
        ...
        //向mq发送视频处理消息
        try {
            rabbitTemplate.convertAndSend(RabbitMQConfig.EX_MEDIA_PROCESSTASK,route_video,string);
